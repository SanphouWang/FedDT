...
class DualModalityMLP(nn.Module):
    def __init__(self, input_dim=90 * 2, output_dim=3):
        super(DualModalityMLP, self).__init__()
        ...
        self.network.apply(initialization)

    def forward(self, mod0=None, mod1=None):
        if mod0 is not None and mod1 is not None:
            x = torch.cat((mod0, mod1), dim=1)
        elif mod0 is not None:
            mod1 = torch.zeros(mod0.size(0), mod0.size(1)).to(mod0.device)
            x = torch.cat((mod0, mod1), dim=1)
        elif mod1 is not None:
            mod0 = torch.zeros(mod1.size(0), mod1.size(1)).to(mod1.device)
            x = torch.cat((mod0, mod1), dim=1)
...
class SingleModalityMLP(nn.Module):
    def __init__(self, input_dim=90, output_dim=3):
        super(SingleModalityMLP, self).__init__()
        self.network = nn.Sequential(
            # First hidden layer
...
# class DualModalityMLP(nn.Module):
#     def __init__(self, input_dim=90, hidden_size=16, num_classes=3):
#         self.hidden_size = hidden_size
#         super(DualModalityMLP, self).__init__()
#         self.fc1_mod0 = nn.Sequential(
#             # First hidden layer
#             nn.Linear(input_dim, 128),
#             nn.ReLU(),
#             nn.Linear(128, 256),
...
#             nn.ReLU(),
#         )
#         self.fc2 = nn.Linear(hidden_size * 2, hidden_size)  # Layer after concatenation
#         self.fc3 = nn.Linear(hidden_size, num_classes)
#         self.dropout = nn.Dropout(0.5)
#         self.relu = nn.ReLU()
#         self.sigmoid = nn.Sigmoid()
#         self.fc1_mod0.apply(initialization)
#         self.fc1_mod1.apply(initialization)

#     def forward(self, mod0=None, mod1=None):
#         try:
#             batch_size = mod0.size(0)
#             device = mod0.device
#         except:
...
#         if mod0 is not None:
#             out_mod0 = self.fc1_mod0(mod0)
#         else:
#             out_mod0 = torch.zeros(batch_size, self.hidden_size).to(device)

#         if mod1 is not None:
#             out_mod1 = self.fc1_mod1(mod1)
#         else:
#             out_mod1 = torch.zeros(batch_size, self.hidden_size).to(device)
#         out = torch.cat((out_mod0, out_mod1), dim=1)
...
def initialization(net):
    ...
    if isinstance(net, nn.Linear):
        init.xavier_uniform_(net.weight)
...
class ADCCA(nn.Module):
    ...
    def __init__(
        self, m1_embedding_list, m2_embedding_list, m3_embedding_list, m4_embedding_list, top_k
    ):
        ...
        self.model1 = nn.Sequential(
            nn.Linear(m1_du0, m1_du1),
            nn.Tanh(),
            nn.Linear(m1_du1, m1_du2),
            nn.Tanh(),
            nn.Linear(m1_du2, m1_du3),
            nn.Tanh(),
        )

        self.model2 = nn.Sequential(
            nn.Linear(m2_du0, m2_du1),
            nn.Tanh(),
            nn.Linear(m2_du1, m2_du2),
            nn.Tanh(),
            nn.Linear(m2_du2, m2_du3),
            nn.Tanh(),
        )

        self.model3 = nn.Sequential(
            nn.Linear(m3_du0, m3_du1), nn.Tanh(), nn.Linear(m3_du1, m3_du2), nn.Tanh()
        )

        self.model4 = nn.Sequential(
            nn.Linear(m4_du0, m4_du1),
            nn.Tanh(),
            nn.Linear(m4_du1, m4_du2),
            nn.Tanh(),
            nn.Linear(m4_du2, m4_du3),
            nn.Tanh(),
        )
    ...
    def cal_loss(self, H_list, train=True):
        ...
        for idx, (f, view) in enumerate(zip(F, views)):
            _, R = torch.qr(view)
            Cjj_inv = torch.inverse(
                (R.T.mm(R) + eps * torch.eye(view.shape[1], device=view.device))
            )
...
if __name__ == "__main__":
    ...
    classifier = DualModalityMLP()
    modality0 = 100 * torch.randn(10, 90)
    modality1 = 100 * torch.randn(10, 90)
    output = classifier(modality0, modality1)
    print(output)
